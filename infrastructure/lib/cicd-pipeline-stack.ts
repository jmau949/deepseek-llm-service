          ...
          phases: {
            pre_build: {
              commands: [
                ...
                "aws ecr get-repository-policy --repository-name llm-service || (echo 'ERROR: Could not check repository policy' && exit 1)",
                ...
                "if ! aws ecr describe-pull-through-cache-rules --query 'pullThroughCacheRules[?ecrRepositoryPrefix==`docker-hub`]' --output text 2>/dev/null | grep -q 'docker-hub'; then",
                "  echo 'ERROR: Pull-through cache rule for docker-hub not found or cannot be verified.'",
                "  echo 'The CDK stack must properly configure dockerHubCache.'",
                "  exit 1",
                "fi",
                "# Verify ECR auth token works",
                "aws ecr get-login-password --region $AWS_REGION | docker login --username AWS --password-stdin $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com",
                "# Test ECR pull-through cache with a small image",
                "echo 'Testing ECR pull-through cache with a small image...'",
                "docker pull $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/docker-hub/busybox:latest || (echo 'ERROR: Failed to pull test image from ECR pull-through cache' && exit 1)",
