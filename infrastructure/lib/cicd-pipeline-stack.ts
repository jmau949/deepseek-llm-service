          ...
          phases: {
            pre_build: {
              commands: [
                ...
                "aws ecr get-repository-policy --repository-name llm-service || echo 'Note: No repository policy found. Using default permissions.'",
                ...
                "if ! aws ecr describe-pull-through-cache-rules --query 'pullThroughCacheRules[?ecrRepositoryPrefix==`docker-hub`]' --output text | grep -q 'docker-hub'; then",
                "  echo 'Pull-through cache rule for docker-hub not found. This must be configured in the CDK stack.'",
                "  echo 'Verify that dockerHubCache is properly configured in CicdPipelineStack.'",
                "  exit 1",
                "fi",
                ...
                "docker pull $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/docker-hub/busybox:latest",
