          ...
          phases: {
            ...
            build: {
              commands: [
                ...
                "else",
                '  echo "ERROR: Dockerfile not found in expected location!"',
            ...
            post_build: {
              commands: [
                ...
                "  cd $CODEBUILD_SRC_DIR", // Return to the source directory
                '  echo "{"ImageURI":"$ECR_REPOSITORY_URI:$IMAGE_TAG"}" > imageDefinition.json',
                "else",
                "  echo Build failed, skipping Docker push",
                "  cd $CODEBUILD_SRC_DIR", // Return to the source directory
                '  echo "{"ImageURI":"$ECR_REPOSITORY_URI:latest"}" > imageDefinition.json',
                "fi",
              ],
            },
          },
